...
bool isSorted(struct LinkedListNode *linkedListCurrentNode)
{
    int linkedListItemCount=INT32_MIN;
    while(linkedListCurrentNode!=NULL)
    {
        if(linkedListCurrentNode->linkedListNodeValue<linkedListItemCount)
            return false;
        linkedListItemCount=linkedListCurrentNode->linkedListNodeValue;
        linkedListCurrentNode=linkedListCurrentNode->linkedListNextLink;
    }
    return true;
}
...
int main()
{
    int linkedListFirstItems[]={4,7,19,12,18};
    create(linkedListFirstItems,5);
    cout<<"Sorted : "<<isSorted(linkedListFirstNode)<<endl;
