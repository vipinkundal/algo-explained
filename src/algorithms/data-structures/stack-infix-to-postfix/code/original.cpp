...
{
    if(stackInfixWorkingText=='*' || stackInfixWorkingText=='/' || stackInfixWorkingText=='+' || stackInfixWorkingText=='-')
        return 0;
    else 
        return 1;
}

int pre(char stackInfixWorkingText)
{
...
char* ToPost(char *stackInfixWorkingText)
{
    ...
    while(stackInfixWorkingText[stackInfixScanIndex]!='\0')
    {
        ...
        {
            if(pre(stackInfixWorkingText[stackInfixScanIndex])>pre(TopValue(stackInfixState2)))//because of this comparison stack cant be expty so # is pushed
...
int main()
{
    char stackInfixWorkingText[]="a+b*c/d+e";
